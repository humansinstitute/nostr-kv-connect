...
class DemoClient {
    ...
    async sendRequest(method, params) {
        ...
        const { encrypted } = await this.dmCrypto.encrypt(requestJson, this.keyring.getSecretKey(), this.serverPubkey);
        ...
        return new Promise((resolve, reject) => {
            ...
            const sub = this.pool.subscribeMany(this.relays, [{
                ...
                onevent: async (event) => {
                    try {
                        // Decrypt response
                        const { decrypted } = await this.dmCrypto.decrypt(event.content, this.keyring.getSecretKey(), event.pubkey);
...
async function main() {
    ...
    if (options.method) {
        ...
        if (options.value)
            params.value = Buffer.from(options.value).toString('base64');
        if (options.ttl)
            params.ttl = parseInt(options.ttl);
        try {
